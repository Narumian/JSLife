...
const outputRoot = join(root, "build-app");
const appName = "JSLIFE.app";
...
const ui = join(resources, "ui");
const executable = join(macOS, "JSLIFE");
...
await Promise.all([
  ...
  cp(join(root, "dist-desktop"), ui, { recursive: true }),
...
<plist version="1.0"><dict>
  <key>CFBundleDisplayName</key><string>JSLIFE</string>
  <key>CFBundleExecutable</key><string>JSLIFE</string>
  <key>CFBundleIdentifier</key><string>studio.jslife.app</string>
  <key>CFBundleInfoDictionaryVersion</key><string>6.0</string>
  <key>CFBundleName</key><string>JSLIFE</string>
...
const dmg = join(outputRoot, "JSLIFE.dmg");
run("hdiutil", ["create", "-volname", "JSLIFE", "-srcfolder", appRoot, "-ov", "-format", "UDZO", dmg]);
