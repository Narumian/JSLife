...
19. デスクトップ版、`npm run dev`によるローカル版、GitHub Pages静的版の3モードへ分離。
...
23. GitHub Pages版の起動チェックを、静的版だからと即座にオフライン扱いにする分岐から、常にローカルブリッジへの接続確認を行う分岐へ変更。同じ端末でローカルブリッジ（`npm run dev`またはJSLIFEデスクトップ版）が起動していれば、Pages版からもCodexチャットを利用できる。未起動時のみ、Release案内とソースからの起動手順を表示する。
...
- **GitHub Pages版**: インストール不要の静的ページ。コード編集、描画、ブラウザ保存、Import・Exportはブラウザ単体で使える。同じ端末でローカルブリッジが起動していればCodexチャットも利用できる。未起動時は最新Release、またはソースからの`npm run dev`起動を案内する。
