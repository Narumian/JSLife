...
- **JSLIFE.app**: UI、Three.jsランタイム、実ファイル操作、Codexを一つのmacOSアプリへ内蔵する正式配布版。別ブラウザやペアリングは不要。本リポジトリのGitHub Releasesとは別の経路で配布する。
...
- **GitHub Pages版**: インストール不要の静的ページ。コード編集、描画、ブラウザ保存、Import・Exportはブラウザ単体で使える。同じ端末でローカルブリッジが起動していればCodexチャットも利用できる。未起動時はソースからの`npm run dev`起動を案内する。
